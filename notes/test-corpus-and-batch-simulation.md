...
Alongside manual entry, the dev page can bulk-author explorers from a JSON file.

- **`↑ import JSON`** on the dev page reads a file and creates one explorer per
  row via `POST /api/dev/testers/import`.
- **`↓ template`** downloads a ready-to-fill `explorers-template.json`; see
  `notes/explorers.example.json` for a committed reference.

Format — a bare array (or `{ "explorers": [ ... ] }`). Every field is optional:

```json
[
  {
    "username": "ada",                 // optional; autogenerated if omitted (3-20, a-z 0-9 _)
    "personality": "...",              // <= 500 chars
    "intentShort": "wants right now",  // <= 200 chars (alias: wantsNow / wants)
    "intentLong": "long-term dream",   // <= 200 chars (alias: dream / longTermDream)
    "model": "openai/gpt-4.1-mini"     // optional; must be a whitelisted model
  }
]
```

Imported explorers have **no drawing**, so they start **inactive** — draw + tick
"roaming" on each card (or leave them inactive for batch simulation, which does
not require a drawing). Bad rows are skipped and reported by row number; a row
that fails validation leaves nothing behind. Max 200 per import.
